...
BEFORE UPDATE ON m_product_group
FOR EACH ROW EXECUTE FUNCTION update_updated_at_column();

-- Product Variant (SKU)
CREATE TABLE m_product (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    product_group_id BIGINT NOT NULL REFERENCES m_product_group(id) ON DELETE RESTRICT,
    store_id INT NOT NULL REFERENCES m_store(id) ON DELETE RESTRICT,
...
CREATE TABLE t_sales_detail (
    id BIGINT GENERATED ALWAYS AS IDENTITY PRIMARY KEY,
    sales_id BIGINT NOT NULL REFERENCES t_sales(id) ON DELETE RESTRICT,
    product_id BIGINT NOT NULL REFERENCES m_product(id) ON DELETE RESTRICT,
